...
def run(args, device, data):
    ...
    for epoch in range(args.num_epochs):
        ...
        # NOTE: since evaluation is currently done on single rank, after the first epoch, the epoch time reported by ranks other than zero include the evaluation time
    ...
    print("time to train: ", end_train_time - start_train_time)
    ...
    if not args.standalone:
        ...
        if g.rank() == 0:
            th.save(node_emb[0 : g.num_nodes()], args.nemb_out)
        ...
        with th.no_grad():
            ...
            th.save(node_emb, args.nemb_out)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="DistSAGE no distrib emb")
    ...
    parser.add_argument(
        "--CSVDataset_dir", type=str, help="input dir with CSVDataset files"
    )
    parser.add_argument(
        "--model_out",
        type=str,
