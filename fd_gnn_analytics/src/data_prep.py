...
def main(args):
    ...
    tic = time.time()
    df = pd.read_csv(args.raw_transaction_data_path)
    ...
    tic = time.time()
    df_merge.to_csv(args.edge_feature_data_path, index=False)
    print(
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="DataPrep")
    parser.add_argument(
        "--raw_transaction_data_path", type=file, help="The path to the raw data file"
    )
    parser.add_argument(
        "--edge_feature_data_path",
        type=str,
